...
contract DMRVManagerFuzzTest is Test {
    ...
    function testFuzz_FulfillVerification(
        ...
        if (updateMetadataOnly) {
            ...
            assertEq(finalHistory.length, initialHistoryLength + 1, 
                "URI history should grow by 1");
            assertEq(finalHistory[finalHistory.length - 1], metadataURI, 
                "New URI should be last in history");
        } else {
            // Balance should increase by the credit amount
            assertEq(credit.balanceOf(projectOwner, uint256(projectId)), initialBalance + creditAmount, 
                "Balance should increase on mint");

            // Metadata should be updated if credits were minted
            if (creditAmount > 0) {
                 assertEq(credit.uri(uint256(projectId)), metadataURI, 
                    "URI should be updated on mint");
                 string[] memory finalHistory = credit.getTokenURIHistory(uint256(projectId));
                 assertEq(finalHistory.length, initialHistoryLength + 1, 
                    "URI history should grow by 1 on mint");
            } else {
                 // If amount is 0 and not update-only, nothing should change
                 assertEq(credit.uri(uint256(projectId)), initialHistory[initialHistory.length - 1], 
