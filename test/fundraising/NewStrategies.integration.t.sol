...
contract NewStrategiesIntegrationTest is Test {
    ...
    function test_DeployAndUseLogarithmicCurve() public {
        // 1. Setup Project in Registry
        bytes32 projectId = keccak256("Test Project Logarithmic");
        projectRegistry.addProject(projectId, projectOwner);
        projectRegistry.setProjectStatus(projectId, IProjectRegistry.ProjectStatus.Active);
        ...
        vm.prank(projectOwner);
        address curveAddress = factory.createBondingCurve(
            projectId, LOGARITHMIC_STRATEGY_ID, "Logarithmic Token", "LOGO", strategyInitializationData
        );
        assertTrue(curveAddress != address(0));

        // 4. Interact with the deployed curve
        LogarithmicCurve deployedCurve = LogarithmicCurve(payable(curveAddress));
        uint256 amountToBuy = 100e18;
        uint256 expectedCost = deployedCurve.getBuyPrice(amountToBuy);
        ...
        assertEq(actualCost, expectedCost, "Buy cost should be correct on deployed logarithmic curve");
        assertEq(deployedCurve.projectToken().balanceOf(user1), amountToBuy, "User should receive project tokens");
    }

    function test_LogarithmicCurve_VestingAndWithdrawal() public {
        // 1. Deploy the curve
        bytes32 projectId = keccak256("Test Vesting Logarithmic");
        projectRegistry.addProject(projectId, projectOwner);
        projectRegistry.setProjectStatus(projectId, IProjectRegistry.ProjectStatus.Active);
        bytes memory initData = abi.encode(
            LOG_PRICE_COEFFICIENT,
        ...
        vm.prank(projectOwner);
        address curveAddress =
            factory.createBondingCurve(projectId, LOGARITHMIC_STRATEGY_ID, "Vesting Token", "VESTLOG", initData);
        LogarithmicCurve deployedCurve = LogarithmicCurve(payable(curveAddress));
        ProjectToken projectToken = deployedCurve.projectToken();
        uint256 ownerInitialCollateral = collateralToken.balanceOf(projectOwner);
        ...
        // 2. Test Vesting: Cannot claim before cliff
        vm.prank(projectOwner);
        vm.expectRevert(abi.encodeWithSelector(LogarithmicCurve__VestingNotStarted.selector));
        deployedCurve.claimVestedTokens();
        ...
        // 6. Second withdrawal fails as it's too soon
        vm.prank(projectOwner);
        vm.expectRevert(abi.encodeWithSelector(LogarithmicCurve__WithdrawalTooSoon.selector));
        deployedCurve.withdrawCollateral();
    }

    function test_AccessControl() public {
        // 1. Deploy a curve (using exponential as the example)
        bytes32 projectId = keccak256("Test Access Control");
        projectRegistry.addProject(projectId, projectOwner);
        projectRegistry.setProjectStatus(projectId, IProjectRegistry.ProjectStatus.Active);
        bytes memory initData = abi.encode(
            EXP_PRICE_COEFFICIENT,
        ...
        vm.prank(projectOwner);
        address curveAddress =
            factory.createBondingCurve(projectId, EXPONENTIAL_STRATEGY_ID, "Access Control Token", "ACT", initData);
        ExponentialCurve deployedCurve = ExponentialCurve(payable(curveAddress));

        // 2. Prank as a random user (user1) and expect reverts
        vm.startPrank(user1);
        string memory expectedRevert = "OwnableUnauthorizedAccount(address)";
        vm.expectRevert(abi.encodeWithSignature(expectedRevert, user1));
        deployedCurve.withdrawCollateral();
    ...
    function test_ExponentialCurve_OverflowRevert() public {
        // 1. Deploy the curve
        bytes32 projectId = keccak256("Test Overflow Exponential");
        projectRegistry.addProject(projectId, projectOwner);
        projectRegistry.setProjectStatus(projectId, IProjectRegistry.ProjectStatus.Active);
        bytes memory initData = abi.encode(
            EXP_PRICE_COEFFICIENT,
            ...
            0,
            0,
            0 // No team allocation for this test
        );
        vm.prank(projectOwner);
        address curveAddress =
            factory.createBondingCurve(projectId, EXPONENTIAL_STRATEGY_ID, "Overflow Token", "OVR", initData);
        ExponentialCurve deployedCurve = ExponentialCurve(payable(curveAddress));

        // 2. Attempt to buy an amount that would cause an overflow in calculation
        uint256 massiveAmount = 3e25; // This will push s2^3 over uint256.max
        vm.startPrank(user1);
        ...
        deployedCurve.buy(massiveAmount, type(uint256).max);
        vm.stopPrank();
    }

    function test_LogarithmicCurve_OverflowRevert() public {
        // 1. Deploy the curve
        bytes32 projectId = keccak256("Test Overflow Logarithmic");
        projectRegistry.addProject(projectId, projectOwner);
        projectRegistry.setProjectStatus(projectId, IProjectRegistry.ProjectStatus.Active);
        bytes memory initData = abi.encode(
            LOG_PRICE_COEFFICIENT,
        ...
        vm.prank(projectOwner);
        address curveAddress =
            factory.createBondingCurve(projectId, LOGARITHMIC_STRATEGY_ID, "Overflow Token Log", "OVRLOG", initData);
        LogarithmicCurve deployedCurve = LogarithmicCurve(payable(curveAddress));

        // 2. Attempt to buy an amount that would cause an overflow in calculation
        uint256 massiveAmount = 3e25; // This will push s^3 over uint256.max
        vm.startPrank(user1);
        collateralToken.approve(curveAddress, type(uint256).max);
        vm.expectRevert(abi.encodeWithSelector(LogarithmicCurve__OverflowRisk.selector));
        deployedCurve.buy(massiveAmount, type(uint256).max);
        vm.stopPrank();
    }
}
